...
- [ ] **Runtime storage switch + dashboard & review client variants**
  - `src/lib/storage/index.ts` picks impl based on `process.env.NEXT_PUBLIC_STORAGE || "prisma"`. Use a `require()` ternary (webpack can DCE on the inlined env var, so the Prisma module never reaches the client bundle in static mode; a static `import` of both impls would pull `PrismaClient` into the browser chunk because `@/lib/db` instantiates it at module load). Default stays `prisma` so dev/server behavior is unchanged.
  - Dispatcher pattern per page: keep `page.tsx` as the existing async server component, but add an early return `if (process.env.NEXT_PUBLIC_STORAGE === "localstorage") return <DashboardClient />;` (same for review). New siblings `src/app/dashboard-client.tsx` and `src/app/review/review-client.tsx` are `"use client"` + `useEffect` that calls the `storage` export and renders the same markup as the server variant. Server path is untouched.
  - Keep `export const dynamic = "force-dynamic"` on both pages for now — the dispatcher short-circuits before it matters in localstorage mode; it'll be dropped in sub-item 5 (build pipeline) alongside `output: 'export'`.
  - Verify: `pnpm tsc --noEmit` clean, `pnpm lint` clean, dev server (`:3000`, `NEXT_PUBLIC_STORAGE` unset) still serves `/` and `/review` at 200 with real Prisma data. Do NOT attempt to boot the localstorage variant yet — static export wiring + `/attempt` fallback come in later sub-items.
  - Commit: `feat: runtime storage switch + dashboard/review client variants`.

- [ ] **Quiz client variant + QuizForm static-mode submit**
  - Dispatcher in `src/app/quiz/page.tsx`: early return `<QuizClient searchParamsPromise={searchParams} />` when `NEXT_PUBLIC_STORAGE === "localstorage"`. New `src/app/quiz/quiz-client.tsx` ports the server logic (topics, getNextDueQuestion, getAggregateCounts, getRecentAttempts, computeSessionStats, cap/exhausted/empty/main states) into a `"use client"` `useEffect` flow. Reads search params via `useSearchParams`.
  - `src/app/quiz/quiz-form.tsx`: replace `fetch("/attempt", …)` with a `mode`-aware submit. In `"localstorage"` mode, call `storage.recordAttempt({ questionId, grade, responseTimeMs })` directly (grade computed in the form: MC → `gradeFromCorrect(userAnswer === correctAnswer, responseTimeMs)`; open → `selfGrade`). In Prisma mode, keep the existing POST. Pick the mode from `process.env.NEXT_PUBLIC_STORAGE` — constant at build time, so the unused branch DCEs.
  - Replace `router.refresh()` in the "Další otázka" path with an `onNext` callback prop. Server variant's `onNext` stays `router.refresh()`. Client variant's `onNext` re-invokes `storage.getNextDueQuestion` + re-computes session stats and re-renders locally (no router).
  - Verify: `pnpm tsc --noEmit` clean, `pnpm lint` clean, all vitest suites still pass (60/60), dev server's `/quiz` flow unchanged (answer MC → grade pill → Další otázka advances), and explicitly verify the Prisma branch is picked when `NEXT_PUBLIC_STORAGE` is unset.
  - Commit: `feat: quiz client variant + static-mode attempt submit`.
